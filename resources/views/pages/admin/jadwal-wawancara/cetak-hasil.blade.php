...
<html lang="en">
	<head>
		...
		<title>Cetak Jadwal Wawancara</title>
		<style>
            .w-100 {
                width: 100%;
            }
	...
	<body>
		...
		<hr />
		<h3 class="text-center">Laporan Hasil Wawancara</h3>
		<table class="table w-100">
            <thead class="text-center" style="background-color: #009e0f; color: #fff;">
                <tr>
                    ...
                    <th id="data" scope="col">Tanggal Wawancara</th>
                    <th id="data" scope="col">Hasil Wawancara</th>
                </tr>
            </thead>
            <tbody>
                @php
                ...
                @foreach ($results as $item)
                    <tr>
                        ...
                        <td id="data">{{ $item->user->tempat_lahir }}/{{ \Carbon\Carbon::parse($item->tgl_lahir)->locale('id')->translatedFormat('d F Y') }}</td>
                        ...
                        <td id="data">{{ \Carbon\Carbon::parse($item->tanggal_wawancara)->locale('id')->translatedFormat('d F Y') }}</th>
                        <td id="data">{{ $item->hasil_wawancara }}</th>
                    </tr>
                @endforeach
            </tbody>
        </table>
